...
function createPixel(
  ...
  const p: Pixel = {
    ...
    maxSizeInt: 2,
    maxSize: rand(0.5, 2),
    restSize: 0, // ponytail: filled by pulse() — floor during breathing cycle
    delay,
    counter: 0,
    ...
    pulse() {
      // ponytail: breathing cycle — radial wave expand→contract→expand forever.
      // Floor at restSize (set once on first tick) so canvas never goes blank.
      p.isIdle = false;
      ...
      // ponytail: pulse step must scale with maxSize — original `speed` was tuned
      // for disappear (per-frame delta), too slow for a visible breathing cycle.
...
function PixelCanvas({ colors, gap = 5, speed = 30, className }: PixelCanvasProps) {
  ...
  useEffect(() => {
    ...
    // ponytail: breathing pulse — radial wave expand→contract→expand forever.
    // Reduced motion → single appear, no loop.
...
export function PixelHero({
  ...
  return (
    <div className="relative w-full min-h-[100dvh] bg-background flex flex-col justify-center md:justify-center gap-7 md:gap-6 py-20 md:py-0 px-4 sm:px-6 overflow-hidden select-none isolate">
      <style>{`
        @keyframes marquee {
        ...
        @media (prefers-reduced-motion: no-preference) {
          .pixel-canvas-fade {
            animation: pixel-fade-in 1.2s ease-out 0.3s both;
          }
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        {themeColors.length > 0 && <PixelCanvas colors={themeColors} gap={6} speed={30} className="pixel-canvas-fade" />}
